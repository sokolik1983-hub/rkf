export const endpointGetExhibitions = '/api/Exhibitions/exhibition/public';
export const endpointGetEducationals = '/api/educations/education/public';
export const endpointExhibitionsFilters = '/api/exhibitions/Exhibition/filter';
export const endpointEducationalsFilters = '/api/educations/education/filter';
export const endpointExhibitionsDates = '/api/exhibitions/Exhibition/dates';
export const endpointExhibitionsCities = '/api/City';
export const endpointExhibitionsSearch = '/api/exhibitions/Exhibition/search';
