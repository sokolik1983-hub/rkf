        ...
        <table>
                ...
                <tbody>
                    {formik.values && formik.values.declarants && Object.keys(formik.values.declarants).map(i => <DocTableItem
                        key={i}
                        activateClick={() => setRedirect(`/kennel/${alias}/documents/litter/${formik.values.declarants[i].id}/declarant/form`)}
                        ...
                        },() => setRedirect(`/kennel/${alias}/documents/litter/${formik.values.id}/table/form`))}}}
                    />)}
                </tbody>
            </table>    
    </Card>
    <div className="stage-controls flex-row">
        <Button className="btn-condensed" onClick={e => setRedirect(`/kennel/${alias}/documents/litter/${formik.values.id}/header/form`)}>Назад</Button>
        <SubmitError />
        <Button className="btn-green btn-condensed" onClick={e => setRedirect(`/kennel/${alias}/documents/litter/${formik.values.id}/payment/form`)}>Продолжить</Button>
    </div>
    </>
)
