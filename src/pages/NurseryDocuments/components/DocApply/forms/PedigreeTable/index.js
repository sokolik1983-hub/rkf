...
// pedigree
const TableFormFields = connect(({formik, update, options, nurseryAlias, setRedirect, send, Title}) => <>
    <Card>
    <Title/>
        ...
        <table>
                ...
                <tbody>
                    {formik.values && formik.values.declarants && Object.keys(formik.values.declarants).map(i => <DocTableItem
                        key={i}
                        activateClick={() => setRedirect(`/nursery/${nurseryAlias}/documents/pedigree/${formik.values.declarants[i].id}/declarant/form`)}
    ...
    <div className="stage-controls flex-row">
            <Button className="btn-condensed" onClick={e => setRedirect(`/nursery/${nurseryAlias}/documents/pedigree/${formik.values.id}/header/form`)}>Назад</Button>
            <SubmitError />
            <Button className="btn-green btn-condensed" onClick={e => setRedirect(`/nursery/${nurseryAlias}/documents/pedigree/${formik.values.id}/payment/form`)}>Продолжить</Button>
    </div>
    </>
)
