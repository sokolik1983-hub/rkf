    ...
    return loading ?
        ...
        <Card className="exhibitions-invite">
            ...
            <div className="exhibitions-invite__main-info">
                ...
                <span>НКП: <a href={`/exhibitions/${mainInfo.nbc_alias}`}>{mainInfo.nbc_name}</a> {judgeStat === 2 ? 'Ваше участие согласовано' : 'В авторизации отказано'}</span>
            </div>
            {/*причина, если отказ*/}
            {judgeStat !== 2 &&
                <div>
                Причина {judgeStatName}
                </div>
            }
            {/*кнопка ОК, если отказ||Принять приглашение и отклонить приглашение, если согласовано*/}
            <div>
                {judgeStat !== 2 ? <Button>Ок</Button> :
                    <div>
