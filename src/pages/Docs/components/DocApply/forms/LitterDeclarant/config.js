...
const config = {
    ...
    onSuccess: {
        save: (values, setRedirect, clubAlias, id) => values && values.id && setRedirect(`/${clubAlias}/documents/litter/${id}/declarant/form`),
        next: (values, setRedirect, clubAlias, id) => values && values.id && setRedirect(`/${clubAlias}/documents/litter/${id}/table/form`)
    },
    options: {
        federations: {
