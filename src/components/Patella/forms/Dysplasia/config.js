...
const validationSchema = distinction => ({...{
    ...
    pedigree_document_id: number().required(reqText).typeError(reqText),
    personal_data_document_id: distinction === 'patella' ? number() : number().required(reqText).typeError(reqText),
...
const config = (distinction, profileType) => ({
    ...
    initialValues: distinction === 'patella' ? {
        express: false,
        federation_id: '',
        declarant_id: '',
